...
BASA <- split(BASA, list(BASA$Season, BASA$Period_of_Week, BASA$Time_of_Day))
...
BASA_fill<-map(BASA, fill_missing)
```

```{r}
# create cluster by 4 hours interval
# List3 stores 24 subset for S2
...
```{r}
BASA_WEEKDAY <- BASA_fill %>% filter(Period_of_Week == "1 - WEEKDAY")

BASA_WEEKEND <- BASA_fill %>% filter(Period_of_Week == "2 - WEEKEND")
```
...
colnames(Result) <- c("Season","Period_of_week","Time_Period","Arrival_rate","Avg_Wait_Time","Service_rate","Avg_servers")
View(Result)
```
...
mean(List3[[2]]$Wait_Time)
mean(List4[[2]]$Wait_Time)
(mean(List3[[2]]$Wait_Time)+mean(List4[[2]]$Wait_Time))/2
```

```{r}
mean(List3[[2]]$Wait_Time)
mean(List3[[3]]$Wait_Time)
mean(List3[[4]]$Wait_Time)
mean(List3[[5]]$Wait_Time)
mean(List3[[6]]$Wait_Time)
```
...
# Regression model
regression_model<- lm(service_rate_perserver ~ arrival_rate_perserver,data = regression_data[-c(1,7,13,19),])
summary(regression_model)
```

$service\_rate\_perserver=0.997245*arrival\_rate\_perserver+0.249409$
$\frac{\mu}{c}=0.997245 \frac{\lambda}{c}+0.249409$
$\mu=0.997245\lambda+0.249409c$
implies that $\hat{a}=0.249409$ and $\hat{b}=0.997245$, where $\lambda$ is the arrival rate, $c$ is the average number of servers, $\mu$ is the service rate.

```{r}
plot(regression_data[-c(1,7,13,19),]$arrival_rate_perserver, regression_data[-c(1,7,13,19),]$service_rate_perserver, pch = 16, col = "blue", xlab = "Arrival rate per server", ylab = "Service rate per server", main = "Regression Line and Points")
...
In our data, we have $\lambda<\hat{\mu}$, QoS level $\mathbb{P}(x)=1-\frac{\lambda}{\hat{a}c+\hat{b}\lambda}e^{-(\hat{a}c+\hat{b}\lambda-\lambda)x}=1-\frac{\lambda}{0.249409c+0.997245\lambda}e^{-(0.249409c+0.997245\lambda-\lambda)x}$

```{r}
# prediction of prob of waittime <x for given cluster related information and time x
predict_prob_waittime <- function(season, PeriodOfWeek, TimePeriod,x){
  match_set <- Result %>% filter(Season == season,Period_of_week == PeriodOfWeek,Time_Period == TimePeriod)
...
```{r}
predict_prob_waittime("Summer","Weekday","12:00:00_16:00:00",15)
```
