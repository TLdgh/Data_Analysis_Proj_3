...
BASA_fill<-map(BASA_fill, function(df){
  df$S2<-as.POSIXct(df$S2, format = "%Y-%m-%d %H:%M:%S")
  df$S1<-df$S2 - as.numeric(df$Wait_Time) * 60 
  return(df)})

sum(is.na(BASA_fill$S1))
sum(is.na(BASA_fill$S2))
```

```{r}
MakeTbl<-function(cluster){
  if(is_empty(unique(cluster$Period_of_Week))==TRUE){
    Result <- data.frame(Season=NaN, Period_of_Week=NaN,Time_of_Day=NaN,Arrival_Rate=NaN,Avg_Wait_Time=NaN,Service_Rate=NaN,Avg_Servers=NaN)
  }else{
    if(unique(cluster$Period_of_Week)=="1 - WEEKDAY"){
    ...
    Result <- data.frame(Season=unique(cluster$Season),Period_of_Week=unique(cluster$Period_of_Week),
                         Time_of_Day=unique(cluster$Time_of_Day),Arrival_Rate=lambda,
...
```{r}
predict_prob_waittime("3 - SUMMER","1 - WEEKDAY","2 - MORNING",15)
```
