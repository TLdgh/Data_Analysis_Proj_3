...
Table 6 told us the probability of waiting time spent in queue is less than 5 minutes up to 30 minutes, increases by 5 minutes, for each cluster respectively. The graph reflects the congestion level of queues at the airport at different times. The faster the curve approaches 1, the shorter the queue time that passengers can expect. For example, we can see from the picture that the airport is busier in the evenings on summer weekdays, and passengers are expected to spend more time waiting in queues. The airport management can also try to add new servers during this time period to alleviate the pressure on traffic. In addition, we notice that the upper part of the graph contains several curves that quickly approach 0. The starting points $(0, \mathbb{P}(x))$ of these curves are close to 1, which indicates that during these times, there are very few or no people in the queue, and passengers can directly pass through the queue and enter the server. This situation mostly occurs at night, possibly due to the low number of passenger arrivals caused by the low number of scheduled flights at the airport during this period, and at the same time, almost all servers at the airport are closed.
